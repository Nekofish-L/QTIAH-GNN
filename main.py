...
def main():
    ...
    category_feat_nums['company'] = [torch.max(h_g.nodes['company'].data['company_feat'][:, i]) + 2 for i in
                                     category_feat_indices['company']]
    category_feat_nums['brand'] = [torch.max(h_g.nodes['brand'].data['brand_feat'][:, i]) + 2 for i in
                                   category_feat_indices['brand']]
    category_feat_nums['organize'] = [torch.max(h_g.nodes['organize'].data['organize_feat'][:, i]) + 2 for i in
                                      category_feat_indices['organize']]
