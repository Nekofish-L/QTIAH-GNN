...
def train(g, model,features, num_epoch, learning_rate, loss_f, device,beta,eta_min):
    ...
    cls_num_list = [(g.nodes['company'].data['y'][train_mask].shape[0]-g.nodes['company'].data['y'][train_mask].sum()).item(),
    ...
    for e in range(num_epoch):
        ...
        loss_func = loss_f(cls_num_list,max_m=0.5,s=10,weight =(1-beta)/(1-beta**torch.tensor(cls_num_list).to(device)),beta = torch.tensor(beta).to(device))
        ...
        val_b_acc = balanced_accuracy_score(labels[val_mask], pred_best[val_mask])
        ...
        optimizer.step()
        scheduler.step()
        print('In epoch {}, loss: {:.3f}, val auc:{.3f} (best:{:.3f}), recall:{:.3f}, bacc:{:.3f}'.format(e, loss,
                                                                                   val_auc,best_val_auc, val_recall, val_b_acc))
