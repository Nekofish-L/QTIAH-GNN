...
def train(g, model,features, num_epoch, learning_rate, loss_f, device,beta,eta_min):
    ...
    for e in range(num_epoch):
        ...
        loss_func = loss_f(cls_num_list,max_m=0.5,s=10,weight =(1-beta)/(1-beta**torch.tensor(cls_num_list).to(device)))
        ...
        optimizer.step()
        scheduler.step()
        print('In epoch {}, loss: {:.3f}, val auc:{:.3f} (best:{:.3f}), recall:{:.3f}, bacc:{:.3f}'.format(e, loss.item(),
